...
#  Preventing the paddle from going out of bounds
    if player2.top <= 0:
        player2.top = 0
    if player2.bottom >= height:
        player2.bottom = height
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.QUIT:
            pygame.quit()
            sys.exit()
        # if score of any player reaches 10 player wins AND game restarts option
        if player1_score == 5 or player2_score == 5:
            ...
            screen.fill((0, 0, 0))
            win_text = font.render(winner_text, True, (255, 255, 255))
